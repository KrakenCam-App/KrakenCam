import { adminFrom } from './adminFetch'

// Load all pricing config from DB
export async function getPricingConfig() {
  const data = await adminFrom('pricing_config', 'select=*')
  const config = {}
...
// Full rows with metadata
export async function getPricingConfigRaw() {
  return adminFrom('pricing_config', 'select=*&order=id')
}
