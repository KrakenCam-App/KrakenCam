...
export function ProjectFilesTab({ project, teamUsers = [], settings = {}, onUpdateProject, onSendFileToDirectMessage, orgId }) {
  ...
  }, [viewerFile?.id, viewerFile?.dataUrl]);
  ...
  const downloadFile = async (file) => {
    ...
    try {
      if (url.startsWith("http")) {
        // Cross-origin Storage URL — must fetch as blob first
        ...
        // data: URL — direct download anchor works fine
        const a = document.createElement("a");
        a.href = url;
        a.download = file.name || "download";
        document.body.appendChild(a);
      ...
      console.warn("[KrakenCam] File download failed:", err.message);
              ...
              <div key={file.id} className="card" style={{ outline: selectedIds.has(file.id) ? "2px solid var(--accent)" : "none", borderRadius:12 }}>
                <div className="card-body" style={{ display:"flex",alignItems:"center",gap:14,flexWrap:"wrap" }}>
                  ...
                  <div style={{ display:"flex",flexWrap:"wrap",gap: isMobile ? 6 : 8,justifyContent: isMobile ? "flex-start" : "flex-end",width: isMobile ? "100%" : "auto" }}>
                    ...
                    {canDownloadFiles
                      ? <button className="btn btn-ghost btn-sm" onClick={() => downloadFile(file)}><Icon d={ic.download} size={13} /> Download</button>
          ...
          <div key={viewerFile.id} style={{ position:"fixed",inset:0,zIndex:9999,background:"rgba(0,0,0,0.93)",display:"flex",flexDirection:"column" }}>
            ...
            <div style={{ flex:1,overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center" }}>
              {!vUrl ? (
                ...
                /* Storage URL PDF — fetch as blob first to avoid Brave/Safari blocking inline rendering */
                pdfBlobUrl === "loading" ? (
                  <div style={{ color:"rgba(255,255,255,.6)",fontSize:14 }}>Loading PDF…</div>
                  ...
                  <div style={{ textAlign:"center",color:"rgba(255,255,255,.7)",padding:40 }}>
                    <div style={{ fontSize:56,marginBottom:16 }}>📄</div>
                    <div style={{ fontSize:15,fontWeight:600,color:"white",marginBottom:8 }}>{viewerFile.name}</div>
                    <div style={{ fontSize:13,marginBottom:24,color:"rgba(255,255,255,.5)" }}>PDF preview unavailable. Open in your browser's PDF viewer.</div>
                    <button className="btn btn-primary" onClick={() => openFile(viewerFile)}><Icon d={ic.arrowUpRight} size={14}/> Open in New Tab</button>
                  </div>
                )
              ) : vIsPdfInline ? (
                /* data: URL PDF — use object tag (works for base64 in Chrome/Firefox) */
